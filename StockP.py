...
today = date.today()
st.write('''#  💵Investment System  📊''' )
# st.sidebar.image("Images/StockStreamLogo1.png", width=250, use_column_width=False)
st.sidebar.write('''# 💵Investment prediction system📊 ''')

with st.sidebar:
    selected = option_menu('STOCK&CRYPTO', ["Stocks Performance Comparison", "Real-Time Stock Price", "Stock Prediction","Cryptos Performance Comparison", "Real-Time Crypto Price", "Crypto Prediction"])
...
Crypto_df = pd.read_csv("CryptoStreamTickersData.csv")

if selected == 'Cryptos Performance Comparison':
    st.subheader("Cryptos Performance Comparison")
    tickers = Crypto_df["Name"]
    dropdown = st.multiselect('Pick your assets', tickers)

    with st.spinner('Loading...'):
        time.sleep(2)

    dict_csv = pd.read_csv('CryptoStreamTickersData.csv', header=None, index_col=0).to_dict()[1]
    symb_list = [dict_csv.get(i) for i in dropdown]
...
elif selected == 'Real-Time Crypto Price':
    st.subheader("Real-Time Crypto Price")
    tickers = Crypto_df["Name"]
    a = st.selectbox('Pick a Company', tickers)
    ...
    if st.button("Search", on_click=callback) or st.session_state.button_clicked:
        if not a:
            ...
            def plot_candle_data():
                fig = go.Figure()
                fig.add_trace(go.Candlestick(x=data['Date'], open=data['Open'], high=data['High'], low=data['Low'], close=data['Close'], name='market data'))
                fig.update_layout(title=f'Candlestick Chart of {a}', yaxis_title='Crypto Price', xaxis_title='Date')
                st.plotly_chart(fig)
...
elif selected == 'Crypto Prediction':
